...
Page {
    ...
    function getResultMessage(scoreRatio) {
        if (scoreRatio >= 0.9) return "Excellent ! Tu es un véritable scientifique en herbe !"
        else if (scoreRatio >= 0.7) return "Très bien ! Tu as de bonnes connaissances scientifiques !"
        else if (scoreRatio >= 0.5) return "Bien joué ! Continue à explorer le monde des sciences !"
        else if (scoreRatio >= 0.3) return "Pas mal, mais tu peux faire mieux. Essaie encore !"
        else return "Continue à t'entraîner, la science est passionnante !";
    }
}
